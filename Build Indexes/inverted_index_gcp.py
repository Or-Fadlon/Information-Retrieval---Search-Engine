...
class MultiFileReader:
    ...
    def open_gcp(self, f_name, work_type):
        client = storage.Client(f_name)
        bucket = client.bucket(self.bucket_name)
        blob = bucket.get_blob('postings_gcp/' + f_name)
    ...
    def read(self, locs, n_bytes):
        ...
        for f_name, offset in locs:
            if f_name not in self._open_files:
                self._open_files[f_name] = self.open_gcp(f_name, 'rb')
...
class InvertedIndex:  
    ...
    def _upload_posting_locs(bucket_id, posting_locs, bucket_name, name):
        ...
        blob_posting_locs = bucket.blob(f"postings_gcp/{name}_index/{bucket_id}_posting_locs.pickle")
